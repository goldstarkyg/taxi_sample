...
class Kohana_Valid {
	...
	public static function min_length($value, $length)
	{
        ...
        //For allowing special chars in password and dis allow space
	 public static function valid_password($value, $expression)
	{
		return (bool) preg_match($expression, (string) $value);
	}
	...
	/** Contact Page Phone Validation **/

	public static function phone1($number)
	{
		
		if(preg_match('/^[0-9-+--\s]{9,16}$/',$number)){
			return true;
		}else{
			return false;
		}
	...
	public static function contact_phone($number)
	{
		
		if(preg_match('/^[a-zA-Z0-9-+()\s]+$/',$number)){
			return true;
		}else{
			return false;
		}
	...
	public static function small_letters($str, $utf8 = False)
	{
	...
	public static function alpha_nospecial($str, $utf8 = FALSE)
	{
		if ($utf8 === TRUE)
		{
			$regex = '/[\'\/~`\!@#\$%\^&\*\(\)_\-\+=\{\}\[\]\|;:"\<\>,\.\?\\\]/';
	...
	public static function alpha_dot($str, $utf8 = FALSE)
	{
	if ($utf8 === TRUE)
	{
	$regex = '/^[-\pL\pN_]++$/uD';
	...
	public static function  must_greaterthan($org_val,$diff_val)
	{
		return ($org_val >= $diff_val);
	}

	public static function  must_lesserthan($org_val,$diff_val)
	{
		return ($org_val <= $diff_val);
	}

	public static function must_lessertotal($org_val,$diff_val)	
	{
		return ($org_val <= $diff_val);
	}
} // End Valid
