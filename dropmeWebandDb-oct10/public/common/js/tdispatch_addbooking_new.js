	...
	function edit_GetElevation(a, dont)
	{
		clearMarker();
		var c = 0;
		var u = 0;
...
function route1(origin_place_id, destination_place_id, travel_mode,directionsService, directionsDisplay) 
{
	...
	var editdrop_placeid = $("#editdrop_placeid").val();
	var editpickup_placeid = $("#editpickup_placeid").val();
	
	if(origin_place_id == editpickup_placeid && destination_place_id == editdrop_placeid){
		var dont = '2';
	}
	console.log(origin_place_id+'|'+editpickup_placeid+'||'+destination_place_id+'|'+editdrop_placeid);
	directionsService.route({
		origin: {'placeId': origin_place_id},
		destination: {'placeId': destination_place_id},
		travelMode: travel_mode
		...
		if (status === google.maps.DirectionsStatus.OK) {
			directionsDisplay.setDirections(response);
			//GetElevation(response.routes[0])
			edit_GetElevation(response.routes[0], dont)
		} else {
			//var j = getDirectionStatusText(g);
			updateStatus("An error occurred calculating the route - " + status);
			working = false
