					...
					<table cellspacing="1" cellpadding="10" width="100%" align="center" class="sTable responsive">
						<thead>
							<tr>
								...
								<td align="left" ><?php echo __('email_template_status'); ?></td>
							</tr>
						</thead>
						<tbody>
							...
							<tr class="<?php echo $trcolor; ?>">
								<td align="center">
									<input type="checkbox" name="uniqueId[]" id="trxn_chk<?php echo $result['email_id'];?>" value="<?php echo $result['email_id'];?>" />
								</td>
								<td align="center"><?php echo $i; ?></td>
								<td align="left"><a href="<?php echo URL_BASE.'edit/email_templates/'.base64_encode($result['email_id']).'/'; ?> "><?php echo $result['email_title']; ?></a></td>
								<td align="left"><?php echo ($result["status"] == 1) ? __("enable") : __("disable"); ?></td>
							</tr>
							<?php $i++; } ?>
						</tbody>
					</table>
					<input type="hidden" name="status" value=""/>					
					<input type="hidden" name="email_sms" value="email"/>					
				</form>
			</div>
		</div>
	</div>
...
$(document).ready(function() {
	...
	$('#more_action').change(function() {
		...
		switch (selected_val) {
			case "status-enable":
				var confirm_msg =  "<?php echo __('Are you sure want to enable Email Template(s)?');?>";
			...
			case "status-disable":
				var confirm_msg =  "<?php echo __('Are you sure want to disable Email Template(s)?');?>";
				if($('input[type="checkbox"]').is(':checked'))
				{
					var ans = confirm(confirm_msg)
					if(ans) {
