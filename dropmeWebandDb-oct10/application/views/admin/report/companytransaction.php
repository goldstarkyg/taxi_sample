...
//$enddate = isset($srch["enddate"]) ? $srch["enddate"] : $enddate;
...
{
	$form_action = URL_BASE.'transaction/companytransaction_list/'.$currentPage.'/';
	$back_action = URL_BASE.'transaction/companytransaction/'.$currentPage.'/';
}
...
<div class="container_content fl clr">
	<div class="cont_container mt15 mt10">
		...
		<table class="list_table1" border="0" width="100%" cellpadding="5" cellspacing="0">
				...
				<tr>
				<td valign="top"><label><?php echo __('passenger_name'); ?></label></td>
						<td valign="top" id="taxi_list">
							<div class="selector ser_input_field" id="uniform-user_type">
...
<?php } else { echo "<div class='nodata'  style='padding:0px 10px 8px 0;'>".__('no_data')."</div></div>"; } ?>
		...
		<?php /* <div class="widget">
            ...
            <div class="over_all mb10">
		<div class="widget">
			<div class="title">
				<h6><?php echo $page_title; ?></h6>
				<div class="exp_menu_right"><?php if($count_transaction_list > 0){ $export_table_count = $count_transaction_list; include_once(APPPATH.'views/admin/transaction_export_menu.php'); } ?></div>
			</div>
		</div>
...
<thead>
	<tr>
		...
		<?php /*<td align="left" width="10%"><?php echo __('admin_commision'); ?></td>
		...
		<?php /* <td align="left" width="5%"><?php echo __('reason');?></td> */ ?>
		<?php } ?>
...
<tbody>	
		...
		<tr class="<?php echo $trcolor; ?>">
			...
			?>
			<td><b><?php echo __('pickup_label'); ?></b><br/><?php echo ($pic_loc != 'No address found')?(substr($pic_loc,0,25).'...'):(substr($pic_loc,0,25));?><br/><b><?php echo __('drop_label'); ?></b><br/><?php echo ($drop_loc != 'No address found')?(substr($drop_loc,0,25).'...'):(substr($drop_loc,0,25));?></td>
			<!--<td><?php //echo $listings['drop_location'];?></td>
			<td><?php //echo $listings['no_passengers'];?></td>-->
			<?php /*<td><?php //echo $listings['admin_amount'];?></td>
		...
		<?php if($list != 'rejected') { ?>
		<tr>
			<?php $colspan = ($list == 'cancelled') ? "10" : "12"; ?>
...
$("#search_user_btn").click(function(){
	...
	if(stFrom != '' && stTo != '' && to_timestamp(stTo) < to_timestamp(stFrom)) {
		$('#enddate_error').html("<?php echo __('enddate_shouldbegreaterthan_startdate'); ?>");
		flag = false;
	}
	return flag;
});
