...
<?php if (isset($error_login)) { ?><span class="login_error"><?php echo $error_login; ?></span><?php } ?>
        <form class="form_login" method="post" name="frmlogin" id="frmlogin">
		    ...
		    <input class="form_control" placeholder="Email address" type="text" name="email" id="email" value="<?php if (isset($_POST['email'])) { echo $_POST['email'];} ?>"  maxlength="45" />
			<?php if(isset($errors) && array_key_exists('email',$errors)){ echo "<span class='error'>".__('please_provide_email')."</span>";}?>
                   <?php /* <div class="rememberMe"><a href="<?php echo URL_BASE; ?>manager/login" class="frgtpsd"><?php echo ucfirst(__('login')); ?></a></div> */?>
                </div>
