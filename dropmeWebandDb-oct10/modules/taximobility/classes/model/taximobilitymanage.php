...
Class Model_TaximobilityManage extends Model
{
	...
	public function unassign_taxi_request($activeids,$date)
	{
		...
		if(count($result) > 0) {
			...
			foreach($result as $a) {
				...
				{
					$this->mongo_db->updateMany(MDB_TAXI_DRIVER_MAPPING,array('_id'=> (int)$mappingId),array('$set'=>array('mapping_enddate' => Commonfunction::MongoDate(strtotime($date)),'mapping_status' => 'U')));
		...
		return (isset($driver_array['unassign_driver_list'])) ? $driver_array['unassign_driver_list'] : array();
		...
		foreach($result as $key => $res)
		{
			...
			if(($res['user_type'] != 'N' && $res['user_type'] !='S') && $res['company_id'] !='')
			{
				...
				if($driver == 1) {
					...
					$details[$key]['total_due_amount_mobile'] = !empty($res['total_due_amount_mobile']) ? $res['total_due_amount_mobile']:'';
					$details[$key]['daily_deduction_amount'] = !empty($res['daily_deduction_amount']) ? $res['daily_deduction_amount']:'';
					$details[$key]['insurance_total_due_amount'] = !empty($res['insurance_total_due_amount']) ? $res['insurance_total_due_amount']:'';
					$details[$key]['insurance_daily_deduction_amount'] = !empty($res['insurance_daily_deduction_amount']) ? $res['insurance_daily_deduction_amount']:'';
   ...
   /* public function getactive_users($companyId = "")
	...
	public function passengerWalletLogs($searchArr, $setPagination=0 , $offset = '', $limit = '')
	{
		...
		$args = array(					
					...
					array('$project' => array(
						...
						'mobile_number' =>  array('$concat' =>  array( '94', '$pass.phone')))),
	...
	public function driverwalletlogs($searchArr, $setPagination=0 , $offset = '', $limit = '')
	{
		$keyword = (isset($searchArr['keyword'])) ? trim(Html::chars($searchArr['keyword'])) : '';
		$startdate = (isset($searchArr['startdate']) && $searchArr['startdate']!="") ? Commonfunction::ensureDatabaseFormat($searchArr['startdate'],1) : '';
		$enddate = (isset($searchArr['enddate']) && $searchArr['enddate']!="") ? Commonfunction::ensureDatabaseFormat($searchArr['enddate'],2) : '';
		...
		if(!empty($keyword) && (!empty($startdate) || !empty($enddate)))
		{
			$match = array( "\$and" => array(array('createdate' => $date),array("\$or"=>array(array('people.name' => Commonfunction::MongoRegex("/$keyword/i")),array( 'people.phone' => Commonfunction::MongoRegex("/$keyword/i") ) ) ) ) );
		}else if(!empty($keyword)){
			$match = array( "\$or"=>array(array('people.name' => Commonfunction::MongoRegex("/$keyword/i")) , 										
				array( 'people.phone' => Commonfunction::MongoRegex("/$keyword/i") ) )   );
		}
		$args = array(					
		...
		$res = $this->mongo_db->Aggregate(MDB_DRIVER_WALLET_AMOUNT_LOG, $args);
		
		if(!empty($res['result'])){
			$i=0;
			foreach($res['result'] as $r){
				$r['date'] = isset($r['date']) ? commonfunction::convertphpdate('Y-m-d H:i:s',$r['date']) : '';
				$r['transaction_id'] = isset($r['transaction_id'])?$r['transaction_id']:'';
				$r['reference_number'] = isset($r['reference_number'])?$r['reference_number']:'';
	...
	public function driverinfo_driverwalletlogs($searchArr, $setPagination=0 , $offset = '', $limit = '')
	{
		//$keyword = (isset($searchArr['keyword'])) ? trim(Html::chars($searchArr['keyword'])) : '';
		$startdate = (isset($searchArr['startdate']) && $searchArr['startdate']!="") ? Commonfunction::ensureDatabaseFormat($searchArr['startdate'],1) : '';
		$enddate = (isset($searchArr['enddate']) && $searchArr['enddate']!="") ? Commonfunction::ensureDatabaseFormat($searchArr['enddate'],2) : '';
		...
		if(!empty($startdate) || !empty($enddate))
		{
			$match = array( "\$and" => array(array('createdate' => $date)));
		}/*else if(!empty($keyword)){
			$match = array( "\$or"=>array(array('people.name' => Commonfunction::MongoRegex("/$keyword/i")) , 										
				array( 'people.phone' => Commonfunction::MongoRegex("/$keyword/i") ) )   );*/
		//}
	...
	public function driver_mobile_deduction_logs( $setPagination=0 , $offset = '', $limit = '',$id='' )
	{
		$limitCnd = "";
		...
		//$match = array('driver_id' => (int)$id, array('daily_deduct_mobile' => array( '$gt' , 0)));
		$match['driver_id'] = (int)$id;
		//$match['balance_mobile_amount'] = array( '$gte' , (int)1);
		//print_r($match);exit();
		$common_arguments = array(
	...
	public function driver_insurance_deduction_logs( $setPagination=0 , $offset = '', $limit = '',$id='' )
	{
		$limitCnd = "";
		$custom_arguments = array();
		if( $setPagination == 1 )
	...
	public function unassign_taxi_list($company = '')
	{
		...
		$args = array(
					...
					'taxi_id' => '$_id')),
					array('$posix_getgroups(oid)'  =>  array('_id'  =>  array('_id' =>  '$id'),
    ...
    {        
        ...
        $args = array(					
					...
					array('$project' => array(
						...
						'transaction_id' => '$transaction_id',
						'createdate' => '$createdate',
						'mobile_number' =>  array('$concat' =>  array( '24', '$pass.phone'))))
				);
		$res = $this->mongo_db->Aggregate(MDB_PASSENGER_WALLET_LOG, $args);
		if(!empty($res['result'])){
			$i=0;
    ...
    {        
        $result = array();
        $res = $this->mongo_db->findOne(MDB_SITEINFO,array(),array("admin_commission","referral_discount","currency_format","referral_amount","referral_settings","wallet_amount1","wallet_amount2","wallet_amount3","wallet_amount_range","insurance_amount"));
        if(!empty($res)){
			$result[] = $res;
		}
        return $result;
    ...
    public function update_journey_statuswith_drop($id, $msg_status, $driver_reply, $travel_status, $drop_latitude, $drop_longitude, $drop_location, $drop_time, $total_distance, $waiting_hours, $tax,$driver_app_version,$usedAmount,$waiting_fare_hour,$fare_per_minute)
    {
        $set_query = array(
            ...
            'company_tax' => $tax,
            'distance' => (double)$total_distance
        );
        $result = $this->mongo_db->updateOne(MDB_PASSENGERS_LOGS,array('_id' => (int)$id),array( '$set'=> $set_query ),array('upsert'=>false));
    ...
    //~ public function get_taximodel($taxiid){
		...
		//~ $result = $this->mongo_db->findOne(MDB_TAXI,array("_id"=>(int)$taxiid),array('taxi_model','taxi_company'));
		//~ return (!empty($result)) ? $result:[];
	//~ }
	
	public function get_taximodel($tripid){
		...
		$args = array(
					...
					array('$project' => array('taxi_model' => '$taxi.taxi_model',
											'taxi_company' => '$taxi.taxi_company',
											'city_name' => '$city_name'
											))
				);
		...
		if(!empty($res['result'])){
			...
			$result_arr['taxi_model'] =  isset($val[0]['taxi_model']) ? $val[0]['taxi_model']:'';
			$result_arr['taxi_company'] =  isset($val[0]['taxi_company']) ? $val[0]['taxi_company']:'';
			$result_arr['city_name'] =  isset($val[0]['city_name']) ? $val[0]['city_name']:'';
		}
		return $result_arr;
	}
}
