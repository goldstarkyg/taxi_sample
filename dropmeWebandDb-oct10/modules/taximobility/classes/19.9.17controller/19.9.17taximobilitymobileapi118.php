	...
	{		
       ...
       if((string)$method=='driver_location_history')
       {
		...
		switch($method)
		{
			...
			$pushMessage = 'Push test message';
			$this->commonmodel->send_pushnotification('eynmSlzjf4Y:APA91bGL1gt45uXIKqUdNTdxEv1ykTYni9-vo_eZH-U69JH9gEnzIyY_zGY84LHj4MM-BPyY3WyraE0_sHh32z6hwObnm19r4uA24k_PdItaHLzVNBhQhuTwfFaV7NhAcctJA-jV2EOR',1,$pushMessage);
			break;
			...
			if(count($config_array) > 0)
			{
				...
				foreach($device_arr as $d){	
					...
					//echo $iOSPassengerLanguageDOC;exit();
			...
			case 'driver_wallet_amount':
				$driver_id = isset($mobiledata['driver_id']) ? $mobiledata['driver_id'] : '';
				if(!empty($driver_id)){
					$driver_wallet = $api->get_driver_wallet_amount($driver_id);
					if(count($driver_wallet) > 0){
						$message = array("wallet_amount" => $driver_wallet[0]['wallet_amount'],"currency" => CURRENCY,"status"=>1);
					} else {
						$message = array("message"=> __('invalid_user'),"status" =>-2);
					}
				} else {
			...
			case 'driver_wallet_addmoney':
			$driver_id = isset($mobiledata['driver_id']) ? $mobiledata['driver_id'] : '';
			$money = isset($mobiledata['money']) ? $mobiledata['money'] : '';
			$p_validator = $this->driver_wallet_addmoney_validation($mobiledata);
			if($p_validator->check())
			{
				$driver_wallet = $this->driver_wallet_addmoney($mobiledata,$default_companyid);
			
			$wallAmount = 0;
			$driwallArr = explode("#",$driver_wallet);
			$wallAmount = isset($driwallArr[1]) ? $driwallArr[1] : 0;
			$driver_wallet = $driwallArr[0];

			if($driver_wallet == 1) {
                
			$message = array("message" => __('amount_added_wallet'), "status"=>1);
					} 

			else if($driver_wallet == 0)
					{
						$gateway_response = isset($wallAmount)?$wallAmount:'Payment Failed';
						$message = array("message" => $gateway_response, "gateway_response" =>$gateway_response,"status"=>0);		
					} else {
			...
			break;
// ********************* update passenger device token start here ***********************************************
				case 'update_passenger_device_token':
					$d_personal_array = $mobiledata;
					if(!empty($d_personal_array))
					{
						$passenger_id = isset($d_personal_array['passenger_id'])?$d_personal_array['passenger_id']:'';		
						$device_token = isset($d_personal_array['device_token'])?$d_personal_array['device_token']:'';	
						if($passenger_id != null)
						{							
							$update_otp = $api->update_passenger_device_token($passenger_id,$device_token);				
							//$message =  array("message" => __('udrop_otp_sent_success'),"status"=>1);	 
				...
				break;
				// ********************* update passenger device token end here ***********************************************
		...
		case 'tripfare_update':
			...
			/** static diver wallet log details **/

			case 'static_wallet_log':

			 $driver_id = isset($mobiledata['driver_id'])?$mobiledata['driver_id']:'';
			 $start = isset($mobiledata['start'])?$mobiledata['start']:'';
			 $limit = isset($mobiledata['limit'])?$mobiledata['limit']:'';
			 $log = $api->driver_wallet_log($driver_id,$start,$limit);	
			 //print_r($log);exit();		
			 $cnt = count($log);			
			 if($driver_id !='' && $log !='') {
			 	$message = array("message" => $log,"count" => $cnt,"status" => 1);
			 } else {
			 	$message = array("message" => "no data","status" => -1);
			 }
			 $mobile_data_ndot_crypt->encrypt_encode_json($message, $additional_param);
			 break;
			 ...
			 case 'mobile_log_details':

			 $driver_id = isset($mobiledata['driver_id'])?$mobiledata['driver_id']:'';
			 $start = isset($mobiledata['start'])?$mobiledata['start']:'';
			 $limit = isset($mobiledata['limit'])?$mobiledata['limit']:'';
			 $log = $api->mobile_deduction_log($driver_id,$start,$limit);			
			 $cnt = count($log['mobile_log']);			
			 if($driver_id !='' && $log !='' && !empty($log['mobile_log'])) {
			 	$message = array("message" => $log['mobile_log'],"count" => $cnt, "status" => 1, "start_date" => $log['start_date'], "end_date" => $log['end_date'], "total_amount_mobile" => $log['total_amount_mobile'], "balance_amount_mobile" => $log['balance_amount_mobile'] );
			 } else {
			 	$message = array("message" => "no data","status" => -1, "start_date" => $log['start_date'], "end_date" => $log['end_date'], "total_amount_mobile" => $log['total_amount_mobile'], "balance_amount_mobile" => $log['balance_amount_mobile'] );
			 }
			 $mobile_data_ndot_crypt->encrypt_encode_json($message, $additional_param);
			 break;

			 /** Insurance log details **/

			 case 'insurance_log_details':

			 $driver_id = isset($mobiledata['driver_id'])?$mobiledata['driver_id']:'';
			 $start = isset($mobiledata['start'])?$mobiledata['start']:'';
			 $limit = isset($mobiledata['limit'])?$mobiledata['limit']:'';
			 $log = $api->insurance_deduction_log($driver_id,$start,$limit);
			 //print_r($log);exit();			
			 $cnt = count($log['insurance_log']);			
			 if($driver_id !='' && $log !='' && !empty($log['insurance_log'])) {
			 	$message = array("message" => $log['insurance_log'],"count" => $cnt,"status" => 1, "start_date" => $log['start_date'], "end_date" => $log['end_date'], "insurance_total_amount" => $log['insurance_total_amount'], "balance_insurance_amount" => $log['balance_insurance_amount']);
			 } else {
			 	$message = array("message" => "no data","status" => -1, "start_date" => $log['start_date'], "end_date" => $log['end_date'], "insurance_total_amount" => $log['insurance_total_amount'], "balance_insurance_amount" => $log['balance_insurance_amount']);
			 }
			 $mobile_data_ndot_crypt->encrypt_encode_json($message, $additional_param);
			 break;
