...
?>],
		...
		{			
			...
			try {
				...
				$insert_array = array(
					...
					"actual_distance" 	=> 0,//urldecode($array['actual_distance']),
    ...
    public function action_getdistancefare(){
		...
		if($tripid != '' && $taxiid != ''){
			...
			$taximodel = $manage_model->get_taximodel($taxiid);
			
			$taxi_fare_details    = $tdispatch_model->get_citymodel_fare_details($taximodel);
