...
/**
 * Kohana Eway payment gateway module
 *
 * @uses       Eway 
 * @package    Custom
 * @author     NDOT Team 
...
class Kohana_Ewaypayment {
    ...
    /**
     * Eway payment gateway connection establishment
     * 
    ...
    public static function connect($connection_string = []) {
        ...
        if (isset($connection_string[0]['payment_method'])) {
            ...
            try {
                // Eway sandbox environment 
                if ($pay_type == "T") {

                    $apiKey = $connection_string[0]['payment_gateway_username'];
                    $apiPassword = $connection_string[0]['payment_gateway_password'];
                    $apiEndpoint = \Eway\Rapid\Client::MODE_SANDBOX;
                }
                //Eway production environment 
                else if ($pay_type == "L") {

                    $apiKey = $connection_string[0]['live_payment_gateway_username'];
                    $apiPassword = $connection_string[0]['live_payment_gateway_password'];
                    $apiEndpoint = \Eway\Rapid\Client::MODE_PRODUCTION;
                } else {
                    throw new Exception('payment method is not valid');
                }

                return $client = \Eway\Rapid::createClient($apiKey, $apiPassword, $apiEndpoint);
     ...
     * @throws Exception
     */
    public static function eway_sale($connection_string = [], $transaction_amount = null, $card_info = [], $shipping_info = [], $additional_parameters = []) {

        $eway_connect = self::connect($connection_string);
        $payment_response = [];
                ...
                $transaction = [
                    'Customer' => [
                        ...
                        'CardDetails' => [
                            ...
                            'ExpiryMonth' => $card_info['expirationMonth'],
                            'ExpiryYear' => $card_info['expirationYear'],
                            'CVN' => $card_info['cvv'],
                        ]
                    ],
                    /* 'ShippingAddress' => [
                      ...
                      'Street1' => $shipping_info['company'],
                      'City' => $shipping_info['city'],
                      'State' => $shipping_info['state'],
                      'PostalCode' => $shipping_info['zipcode'],
                      'Mobile' =>$shipping_info['phone'],
                      ], */
                    'Payment' => [
                        'TotalAmount' => $transaction_amount * 100,
                    ],
                    'TransactionType' => \Eway\Rapid\Enum\TransactionType::PURCHASE,
                    'Capture' => true
                ];

                $response = $eway_connect->createTransaction(\Eway\Rapid\Enum\ApiMethod::DIRECT, $transaction);
//~ echo '<pre>';print_r($response);exit;
                if ($response->TransactionStatus == 1) {
                    $payment_response['payment_response'] = $response->ResponseMessage;
                    $payment_response['CORRELATIONID'] = $response->AuthorisationCode;
                    $payment_response['TRANSACTIONID'] = $response->TransactionID;
                    $payment_response['payment_status'] = 1;
                    $payment_response['cardType'] = "";
                } else {

                    $payment_response['payment_status'] = 0;
                    $error_message = self::getErrorMessage($response->Errors);
                    //~ $payment_response['payment_response'] = $response->Errors;
                    $payment_response['payment_response'] = $error_message;
                }
            } catch (Exception $e) {
            ...
            throw new Exception('Eway payment Not properly configured or Eway payment gateway not active');
        }
    }

    /**
     * Eway payment preauthorization transaction 
     * 
     * @param type $connection_string
     * @param type $transaction_amount
     * @param type $card_info
     ...
     * @throws Exception
     */
    public static function eway_preauthorization($connection_string = [], $transaction_amount = null, $card_info = [], $shipping_info = [], $additional_parameters = []) {
            ...
            try {
                ...
                $transaction = [
                    ...
                    /* 'ShippingAddress' => [
                      ...
                      'LastName' => $lastname,
                      'Street1' => $company,
                      'City' => $city,									'State' => $shipping_info['state'],
                      'PostalCode' => $zipcode,
                      'Mobile' =>$phone,
                      ], */
                    'Payment' => [
     ...
     * @throws Exception
     */
    public static function eway_settlement($connection_string = [], $transactionId = '', $transact_param = []) {
        ...
        $payment_response['payment_method'] = isset($connection_string[0]['payment_method']) ? $connection_string[0]['payment_method'] : '';
        $payment_response['payment_gateway_id'] = isset($connection_string[0]['payment_gateway_id']) ? $connection_string[0]['payment_gateway_id'] : '';
        $amount = $transact_param['preTransactAmount'];
        //Try to submit a Card Payment
        if ($eway_connect->isValid() == 1) {
            ...
            try {
                ...
                $response = $eway_connect->createTransaction(\Eway\Rapid\Enum\ApiMethod::AUTHORISATION, $transaction);
     ...
     * @throws Exception
     */
    public static function eway_void($connection_string = [], $transactionId = '') {
        $eway_connect = self::connect($connection_string);
        $payment_response = [];
        $invoice = substr(mt_rand() . microtime(), 0, 10);
        $payment_response['payment_method'] = isset($connection_string[0]['payment_method']) ? $connection_string[0]['payment_method'] : '';
        ...
        if ($eway_connect->isValid() == 1) {
            ...
            try {
                ...
                $response = $eway_connect->cancelTransaction($transactionId);
    ...
    {
    	$err_msgs = array("A2000"=>"Transaction Approved",
		...
		return isset($err_msgs[$error_code]) ? $err_msgs[$error_code] : "Some Error occurs; Please check all details given;";
    }
    //~ $error_message = self::getErrorMessage($response->Errors);
